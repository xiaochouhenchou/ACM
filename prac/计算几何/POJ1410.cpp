...
void solve() {
	...
	if(cmp(p[0].x, p[2].x) == 1) swap(p[0].x, p[2].x);		// > 
	if(cmp(p[0].y, p[2].y) == -1) swap(p[0].y, p[2].y);		// <
	p[1].x = p[2].x;
	p[1].y = p[0].y;
	p[3].x = p[0].x;
	p[3].y = p[2].y;

	int ok = (cmp(p[0].x, min(l.x, r.x)) <= 0 && 
			cmp(max(l.x, r.x), p[2].x) <= 0 &&
			cmp(p[2].y, min(l.y, r.y)) <= 0 &&
			cmp(max(l.y, r.y), p[0].y) <= 0);		// 线段在矩形内部
	for(int i = 0; i < 4; i++) {
		Point a = p[i], b = p[(i + 1) % 4];
		ok |= segment_intersection(a, b, l, r);
	}
